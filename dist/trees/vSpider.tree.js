...
class VerticalSpider {
    ...
    map_children_data_to_head(parentSVGEl, parentId, provided_hierarchy) {
        ...
        hierarchies.forEach(head => {
            ...
            if (parentSVGEl === undefined) {
                ...
                if (this.getElemRelPosInTree(head.id) % 2 == 0) { // el position relative to root parent is even
    ...
    handleCollapseRootElChildren(svgNode, id, clicked_pos) {
        ...
        if (clicked_pos == 0) {
            immediate_root_children = this.tree_data.filter(data => data.parentId == id && this.getElemRelPosInTree(data.id) % 2 == 0);
        }
        else {
            immediate_root_children = this.tree_data.filter(data => data.parentId == id && this.getElemRelPosInTree(data.id) % 2 != 0);
        }
